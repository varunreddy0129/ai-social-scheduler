...
import { Calendar, CalendarIcon, XIcon ,ClockIcon ,ArrowRightIcon ,CalendarDaysIcon, SendIcon} from "lucide-react";

const Scheduler = () => {
    
    const[posts, setPosts] = useState<any[]>([]);
    const[content,setContent] = useState(""); 
    const[scheduledDate, setScehduledDate] = useState("");
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input type ="date" required className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 text-sm outline-none" value ={scheduledDate} onChange ={(e) => setScehduledDate(e.target.value)}/>
                                </div>
                            </div>
                            <div>
            ...
            <div className="flex-1 flex flex-col gap-6 min-w-0">
                ...
                <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                    <div className="flex items-center gap-2.5 px-5 py-4 border-b border-slate-100">
                        ...
                        <span className="ml-auto text-sx font-bold bg-zinc-100 text-zinc-700 px-2 py-0.5 rounded-full">{scheduled.length}</span>
                                ...
                                <div key ={post._id} className ="px-5 py-4 hover:bg-slate-50/60 transition-colors">
                                    ...
                                    <div className="flex items-center justify-between mb-2">
                                        ...
                                        <div className="flex items-center gap-2">
                                            {post.mediaType && <span className="text-sx bg-slate-100 text-slate-600 border border-slate-200 px-1.5 py-0.5 rounded-md font-semibold capitalize">{post.mediaType}</span>}

                                            <span className="text-sx text-slate-400">{new Date(post.scheduledFor).toLocaleString()}</span>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                    <div className="flex items-center gap-2.5 px-5 py-4 border-b border-slate-100">
                        ...
                        <span className="ml-auto text-sx font-bold bg-zinc-100 text-zinc-700 px-2 py-0.5 rounded-full">{published.length}</span>
                                ...
                                <div key ={post._id} className ="px-5 py-4 hover:bg-slate-50/60 transition-colors">
                                    ...
                                    <div className="flex items-center justify-between mb-2">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-sx text-slate-400">{new Date(post.updatedAt).toLocaleString()}</span>
                                            <span className="text-xs bg-emerald-50 textt-emerald-700 border border-emerald-100 px-2 py-0.5 rounded-full">Published</span>
