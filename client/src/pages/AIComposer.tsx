...
const AIComposer = () => {
    ...
    const[scheduledTime,setScehduledTime] = useState("");
        ...
        <div className="max-w-4xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
            ...
            <div className="space-y-6 text-center mt-20">
                ...
                <div className="relative group mt-12">
                  <textarea rows={6} placeholder="Share your idea... (e.g. A post about the launch of our new eco-friendly coffee beans)" value={prompt} onChange={(e) => setPrompt(e.target.value)} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-5 py-4 text-sm text-slate-700 placeholder-slate-400 outline-none resize-none"/>
                ...
                <div className="flex flex-wrap justify-center gap-2">
                    {tones.map((t) => (
                        <button key ={t} onClick={() => setTone(t)} className={`px-4 py-1.5 rounded-full text-sm transition-all border ${tone === t ? "bg-red-500 border-red-500 text-white" :"bg-white border-slate-200 text-500 hover:border-slate-300"}`}>
                            {t}
                        </button>
                    ))}
            ...
            <div className="space-y-6 pt-12 border-t border-slate-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {generations.map((gen) => (
                        <div key={gen._id} className="group bg-white rounded-2xl border border-slate-100 p-5 hover:border-red-200 tansition-all relative overflow-hidden">
                    ...
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">
                                ...
                                {PLATFORMS.map((P) => {
                                    ...
                                    return (
                                        <button key={P.id} onClick={() => setSelectedPlatforms((prev) => (
                                            prev.includes(P.id) ? prev.filter((x) => x !== P.id) : [...prev,P.id]))} className={`p-2.5 rounded-md border text-xs ${active ? "bg-red-500/80 text-white" :"bg-white border-slate-200 text-slate-400 hover:border-slate-300"}`}>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div className="relative">
                                ...
                                <input type = "time" className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-md text-slate-900 text-sm focus:outline-none transition-all" value={scheduledTime} onChange={(e) => setScehduledTime(e.target.value)} />
                            </div>
