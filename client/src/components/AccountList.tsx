...
const AccountList = ({accounts, onDisconnect}: AccountListProps ) => {
    ...
    const handleDisconnect = async (accountId: string) => {
        const confirm = window.confirm("Are you sure you want to disconnet this account?");
        if(!confirm) return;
    ...
    if(accounts.length === 0){
        return (
            <div className="bg-white rounded-2xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center py-20 px-6">
                ...
                <p className="text-sm text-slate-400 mt-1 max-w-xs text-center">Connect your first social platform to start scheduling your connect.</p>
            </div>
        )
    }
    return (
